           ...
           <tbody class="text-nowrap">
               ...
               <center><input type="checkbox" class="delete_checkbox" value="<?= $row->id; ?>" id="multiple_delete<?= $row->id; ?>" />
                   <label for="multiple_delete<?= $row->id; ?>"></label></center>  
                    ...
                    <td class="sr_no"><?=++$i?></td>
                    <td><?=" ( ".$row->tehsil_code .' ) '. $row->tehsil_name?></td>
                    <td><?=$row->name?></td>
                    <td><?=$row->code?></td>
                    <td class="text-center">
                       <span class="changeStatus" data-toggle="change-status" value="<?=($row->active==1) ? 0 : 1?>" data="<?=$row->id?>,ward-block,id,active" ><i class="<?=($row->active==1) ? 'la la-check-circle' : 'icon-close'?>" title="Click for chenage status"></i></span>
                   </td>
                   <td class="text-center">
                   <input type="number" value="<?=$row->seq;?>" data="<?=$row->id;?>,ward-block,id,seq" class="change-indexing" min="0" style="text-align:center;width:80px">
                    ...
                    <td> 
                       <a href="javascript:void(0)" data-toggle="modal" data-target="#showModal" data-whatever="Update Ward Block Name  (<?=$row->name?>)" data-url="<?=$update_url?><?=$row->id?>" title="Update Ward Block Name">
                           <i class="la la-pencil-square"></i>
                       </a>

                       <a href="javascript:void(0)" onclick="_delete(this)" url="<?=$delete_url?><?=$row->id?>" title="Delete Ward Block Name" >
